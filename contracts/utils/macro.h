/**
 * These are helper macros for writing hooks, all of them are optional as is
 * including hookmacro.h at all
 */
...
#define DONEEMPTY() accept(0, 0, __LINE__)

#define DONEMSG(msg) accept(msg, sizeof(msg), __LINE__)

#define DONE(x) accept(SVAR(x), (uint32_t)__LINE__);
...
#define TRACEVAR(v)                                                            \
  if (DEBUG)                                                                   \
    trace_num((uint32_t)(#v), (uint32_t)(sizeof(#v) - 1), (int64_t)v);
#define TRACEHEX(v)                                                            \
  if (DEBUG)                                                                   \
...
#define TRACEXFL(v)                                                            \
  if (DEBUG)                                                                   \
    trace_float((uint32_t)(#v), (uint32_t)(sizeof(#v) - 1), (int64_t)v);
#define TRACESTR(v)                                                            \
  if (DEBUG)                                                                   \
    trace((uint32_t)(#v), (uint32_t)(sizeof(#v) - 1), (uint32_t)(v),           \
          sizeof(v), 0);
...
#define UINT8_FROM_BUF(buf) (((uint8_t)((buf)[0]) << 0))
...
#define UINT16_FROM_BUF(buf)                                                   \
  (((uint64_t)((buf)[0]) << 8) + ((uint64_t)((buf)[1]) << 0))
...
#define INT64_FROM_BUF(buf)                                                    \
  ((((uint64_t)((buf)[0] & 0x7FU) << 56) + ((uint64_t)((buf)[1]) << 48) +      \
    ((uint64_t)((buf)[2]) << 40) + ((uint64_t)((buf)[3]) << 32) +              \
    ((uint64_t)((buf)[4]) << 24) + ((uint64_t)((buf)[5]) << 16) +              \
    ((uint64_t)((buf)[6]) << 8) + ((uint64_t)((buf)[7]) << 0)) *               \
   (buf[0] & 0x80U ? -1 : 1))
